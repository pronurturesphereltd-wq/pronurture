...
| 2026-06-08 | Added comment moderation queues to Sanity Studio structure — Blog → 💬 Comments → ⏳ Pending Approval / ✓ Approved; filtered document lists with defaultOrdering | Studio structure |
| 2026-06-08 | Removed `TestimonialsSection` from homepage; replaced with `WaitlistSocialProof` — two-part section: dark green waitlist counter banner (count from `siteSettings.waitlistCount`) + brand-light problem statistics grid (3 sourced Nigeria health workforce stats) | Homepage `/` |
| 2026-06-08 | Added `waitlistCount` number field (initialValue: 0) to `siteSettings` schema; added to `siteSettingsQuery` and `SiteSettings` type; seeded via `npm run seed:settings` | Sanity CMS, siteSettings |
| 2026-06-08 | Removed `testimonials` array field from `homePage` schema; removed from `homePageQuery` and `HomePageData` type — testimonial documents remain for use on employers/professionals pages | Sanity CMS, Homepage |
